...
export async function loginInstagram(username, password) {
  ...
  // await ig.simulate.preLoginFlow(); // Often causes issues and flags the account

  // O Instagram valida fortemente os headers e IP de login. Como estamos rodando em uma API,
  // pode ser que IP de datacenter de block direto "IgLoginInvalidUserError".
  // Aqui estamos pelo menos garantindo a geracao do device antes.
  // Muitas vezes, logar uma vez no app oficial com esse IP e confirmar que "fui eu" resolve o InvalidUser.
  ...
  try {
    ...
    if (error.name === 'IgCheckpointError') {
      ...
      throw new Error("O Instagram solicitou verificacao (Checkpoint). Verifique seu email/sms e tente novamente ou confirme no app.");
    } else if (error.name === 'IgLoginTwoFactorRequiredError') {
      throw new Error("Autenticacao de dois fatores requerida. Nao suportado via API simples no momento.");
    }
